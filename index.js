...
const fs = require("fs");
const COOKIE = "PREF=f4=4000000&tz=Europe.Zagreb; CONSENT=PENDING+362; YSC=70h7u2E4Pxw; SID=Rggb3obz5pFH0JJdoDhbBjrMQZm-mKG0_LrHwHgGLAGQtmqevaB7eDFiD40jRqV0QAKHlg.; __Secure-1PSID=Rggb3obz5pFH0JJdoDhbBjrMQZm-mKG0_LrHwHgGLAGQtmqeFIyHRMgLQ5ayUnrETZZx2A.; __Secure-3PSID=Rggb3obz5pFH0JJdoDhbBjrMQZm-mKG0_LrHwHgGLAGQtmqelrf5IHfWLpHnsB-4XnSSlw.; HSID=ACU3hsOPbSc8hMI03; SSID=A2k1mnX3xDfnP8Vtj; APISID=SFFfvtuivpZqVDe8/A6ItFNItzX2T4hiot; SAPISID=K5SuQ0Xs4FSKk416/Ab6LJ8GUtS0ZyPN7n; __Secure-1PAPISID=K5SuQ0Xs4FSKk416/Ab6LJ8GUtS0ZyPN7n; __Secure-3PAPISID=K5SuQ0Xs4FSKk416/Ab6LJ8GUtS0ZyPN7n; LOGIN_INFO=AFmmF2swRgIhALqH_GEIMHX5pQiev-N6E7OwJGNRFcV6MvFiOeeoBrLFAiEA4L8ZkYRQL65SJDN7PIIVMizzNuWs2yNa-CnQdxDR5cM:QUQ3MjNmeW0xY0RiV1JiNnZ6Y1RNVmNyTUl0MjltY3FiZElBTnpkdU5WSklDVlo1bi1oX0RKbGVjYmFuV1hSaWs3aUhaVVRJblVUVXNiZGFlR2taOXpremNhamJFdExHOUpHaGNOOGVhYlJvSlZLU1ItdVR0U1FLR3ZBZzcySmdZVWhTMVdBOUQ5bUFERFhlbXZkSU8tNnFla003b3huN2tB; __Secure-YEC=Cgt1bnR3V1puMWFpVSj4xeGcBg%3D%3D; SIDCC=AIKkIs02UhJhohXeglTxWbxaw3F-ab9g-o465y4kk4c8t-hMpv_PopirsZrYLAJ8DloRvR-3; __Secure-1PSIDCC=AIKkIs3qQK7Vx-Ix6Aw2tnN08enMG9bZWlfBybLSt6_t8ZHq_DHPNYZ37GkvZlHkBJhTVjfM; __Secure-3PSIDCC=AIKkIs3gaKwc0eeUOzBLam7p-jeqZkllRaiUy_8do-kXinx5RoCD76Yqk0dYr1zWCje4aCKd"
  ...
  if (msg.content[0] == "$") {
    ...
    switch (command[0]) {
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              msg.reply("Error 403 or 410, YouTube blocked the bot. Restarting now. Please inform the owner of this bot.")
              await logit("RESETTING CAUSE 410")
              await logit("[" + msg.guild.name + "]" + "Search engine returned nothing.")
              await logit(e.toString())
              process.kill(1)
      ...
      case "goto":
        ...
        break;
      case "die":
        logit("Die command issued, restarting.")
        process.kill(1)
...
async function playAudio(ytlink, playerr) {
  ...
  try {
    let audiostream = ytdl(ytlink, {
      filter: "audioonly", quality: 'highestaudio', format: 'webm', highWaterMark: 1 << 27, requestOptions: {
        headers: {
          cookie: COOKIE
        }
      }
    ...
    logit("Fucking fuck, retrying:" + e.toString())
    let audiostream = ytdl(ytlink, {
      filter: "audioonly", quality: 'highestaudio', format: 'webm', highWaterMark: 1 << 27, requestOptions: {
        headers: {
          cookie: COOKIE
